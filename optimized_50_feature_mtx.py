...
###############################################################################################################################################################################
    ...
    for i in range(rows):
        out = ecg.ecg(signal=ECG_samples_proc[i], sampling_rate=700, show=False)
        r_peaks = out['rpeaks']
...
output.columns = ['ID', 'HRV', 'HR', 'NN20', 'NN50', 'RMSSD', 'SDSD', 'meanRamp', 'HRbio', 'sdnnbio', 'rmssdbio',
                  ...
                  'scr_time_ratio', 'scr_tonic_ratio', 'scr_area_peaks', 'Label']

output.to_csv('C:/Users/tjges/OneDrive/Documents/EPO4/Optimized_50_features/out_window_45.csv', index=False)
# %%
